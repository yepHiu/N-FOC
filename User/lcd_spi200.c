//
// Created by yephi on 11/26/2024.
//
/***
	************************************************************************************************************************************************************************************************
	*	@version V1.0
...
void HAL_SPI_MspInit_(SPI_HandleTypeDef* hspi)
{
...
{
    MX_SPI6_Init_();               // 初始化SPI和控制引脚
    HAL_SPI_MspInit_(&hspi6);
...
void LCD_PutChar(char c) {
    static uint16_t cursor_x = 0; // 当前光标位置
    static uint16_t cursor_y = 0;

    // 处理换行
    if (c == '\n') {
        cursor_x = 0;
        cursor_y += FONT_HEIGHT; // 假设字体高度是 FONT_HEIGHT
        return;
    }
    ...
    // 绘制字符到屏幕
    LCD_DisplayChar(cursor_x, cursor_y, c);
    LCD_DisplayString(cursor_x, cursor_y,&c);

    // 更新光标
    cursor_x += FONT_WIDTH; // 假设字体宽度是 FONT_WIDTH
    if (cursor_x >= LCD_WIDTH) { // 超出一行时换行
        cursor_x = 0;
        cursor_y += FONT_HEIGHT;
    }

    // 如果超出屏幕底部，可以实现滚动功能或清屏
    if (cursor_y >= LCD_HEIGHT) {
        cursor_y = 0; // 示例中直接重置为顶部
        LCD_Clear();
    }
}
